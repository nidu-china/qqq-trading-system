...
class StrategyEngine:
    ...
    def _cache_reversal_indicators(
        ...
        cache_size = max(RULES.timed_reversal_window, RULES.timed_trend_cross_lookback)
        first = max(0, len(bars) - cache_size)
        macd_required = RULES.timed_macd_slow + RULES.timed_macd_signal - 1
        histogram = (
            macd_histogram_series(
                closes,
        ...
        for index in range(first, len(bars)):
            ...
            if len(prefix) >= RULES.timed_boll_period:
                _, middle, _ = bollinger_bands(
                    prefix, RULES.timed_boll_period, RULES.timed_boll_stddev
                )
                self._last_boll_middle_by_end[bars[index].end] = middle
            histogram_index = index - macd_required + 1
            if histogram_index >= 0:
                self._last_macd_hist_by_end[bars[index].end] = histogram[histogram_index]
    ...
    def _prior_boll_middle_crosses(trend_sides: Sequence[bool]) -> int:
        ...
        return sum(
            ...
            for previous_side, current_side in zip(prior_sides, prior_sides[1:], strict=False)
        )

    def _effective_boll_middle_crosses(self, trend_sides: Sequence[bool]) -> int:
    ...
    def _one_minute_context(
        ...
        upper, middle, lower = bollinger_bands(
            closes, RULES.timed_boll_period, RULES.timed_boll_stddev
        )
        ...
        _, two_bars_ago_middle, _ = bollinger_bands(
            closes[:-2], RULES.timed_boll_period, RULES.timed_boll_stddev
        )
        macd_line, macd_signal, macd_hist = macd_histogram(
            closes, RULES.timed_macd_fast, RULES.timed_macd_slow, RULES.timed_macd_signal
        )
        _, _, previous_hist = macd_histogram(
            closes[:-1], RULES.timed_macd_fast, RULES.timed_macd_slow, RULES.timed_macd_signal
        )
    ...
    def _mean_reversion_bar_exit(self, position: Position) -> ExitDecision | None:
        ctx = self.last_context
        if ctx is None:
            return None
        if position.direction is Direction.CALL:
            ...
            macd_on_reversal_side = ctx.macd_hist >= ZERO
            reversal_candle = ctx.current_close > ctx.current_open
        volume_confirmed = ctx.rvol_val >= REVERSAL_VOLUME_STRONG or (
            ctx.rvol_val >= REVERSAL_VOLUME_BASE and ctx.rvol_val > ctx.rvol_prev
        )
        if macd_reversed:
            ...
            if not macd_on_reversal_side:
                position.macd_reversal_pending = False
                position.macd_reversal_pending_at = None
            elif ctx.bar_end != position.macd_reversal_pending_at and volume_confirmed:
                position.macd_reversal_pending = False
                position.macd_reversal_pending_at = None
                return ExitDecision(ExitReason.DIRECTION_REVERSAL, position.quantity)

        bars_after_entry = [bar for bar in self._today_bars if bar.end > position.opened_at]
        if len(bars_after_entry) < 2:
            return None
        window = bars_after_entry[-2:]
        boll_values = [self._last_boll_middle_by_end.get(bar.end) for bar in window]
        if any(value is None for value in boll_values):
            return None
        if position.direction is Direction.CALL:
            boll_condition = all(
                bar.close < middle for bar, middle in zip(window, boll_values, strict=True)
            )
        else:
            boll_condition = all(
                bar.close > middle for bar, middle in zip(window, boll_values, strict=True)
...
# Previous module/class names. Prefer StrategyEngine.
HybridEngine = StrategyEngine
AdaptiveEngine = StrategyEngine

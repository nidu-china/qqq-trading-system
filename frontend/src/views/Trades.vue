...
const { loadLabels, exitReasonLabel, exitReasonType } = useLabels()
...
onMounted(async () => { await Promise.all([load(), loadLabels()]) })
</script>
<template>
  <div class="panel">
  ...
  <el-drawer v-model="drawer" title="交易详情" size="430px"><div v-if="detail" class="kv"><div v-for="(value,key) in detail" :key="key"><label>{{ key }}</label><b>{{ key === 'exit_reason' ? exitReasonLabel(String(value)) : String(value??'—') }}</b></div></div></el-drawer>
</template>
